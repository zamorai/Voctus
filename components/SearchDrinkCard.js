...
export default function DrinkCard({ hits }) {
  return (
    <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {hits.map((drink) => (
        <li
          key={drink.idDrink || drink.id}
        ...
        >
          ...
          <div className="flex-1 flex flex-col p-8">
            <img className="w-56 h-56 flex-shrink-0 mx-auto rounded-md" src={drink.strDrinkThumb || drink.imageURL} alt="" />
            <h3 className="mt-6 text-gray-900 text-sm font-medium">{drink.strDrink || drink.name}</h3>

            <dl className="mt-1 flex-grow flex flex-col justify-between">
              <dt className="sr-only">Title</dt>
              <dd className="text-gray-500 text-sm">{drink.strIngredient1 || drink.ingredient1}</dd>
          ...
          <div className="-mt-px flex divide-x divide-gray-200">
            <div className="-ml-px w-0  flex-1 flex">
              <Link href={`/drinks/${drink.idDrink || drink.id}`}> 
              <a
